...
function StoryCard({ panels }: StoryCardProps): JSX.Element {
  return (
    ...
    >
      <Image
        src="/images/img11.png" // your full combined image
        alt="comic strip"
        width={1200}
        height={460}
...
function CollectionCard({ item }: CollectionCardProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      >
        <h2
          className="text-white font-display leading-none mb-1
             md:mt-0 mt-2"
          style={{
...
function ChapterGroup({
  ...
  return (
    <div ref={groupRef} className="relative  mb-16">
      ...
      >
        ...
        >
          <div className="text-4xl font-display text-white text-left md:text-left">
            <span className="block md:hidden absolute left-[calc(100%+40px)] whitespace-nowrap">
              {group.chapter}
            </span>
  ...
  `}
      ...
      {group.collection && (
        <div
          style={{ paddingLeft: `${collLeft}px`, marginTop: `${VERT_GAP}px` }}
        >
          <motion.div
            ref={collMeasureRef}
...
export default function StaggeredTimeline(): JSX.Element {
  const scrollRef = useRef<HTMLDivElement>(null);

  // Scroll to bottom on mount (chat-like behaviour)
  useEffect(() => {
  ...
  // Responsive geometry — derived from container width via CSS custom properties
  // We use a fixed pixel approach with clamp-like values via JS:
  // On mobile (<480): spineX=40, storyLeft=56, collLeft=80
  // On tablet (480-768): spineX=56, storyLeft=76, collLeft=108
  // On desktop (≥768): spineX=72, storyLeft=96, collLeft=140
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (w < 480) {
        setDims({ spineX: 38, storyLeft: 54, collLeft: 72 });
      } else if (w < 768) {
        setDims({ spineX: 52, storyLeft: 72, collLeft: 100 });
      } else {
        setDims({ spineX: 72, storyLeft: 96, collLeft: 140 });
      }
    };
    update();
  ...
  return (
    <div className="max-w-4xl mx-auto h-auto flex flex-col  py-8  ">
      ...
      >
        <div
          ref={containerRef}
          className="relative mx-auto px-3 sm:px-4 pb-16 pt-4"
