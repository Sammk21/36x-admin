...
export default class StrapiModuleService {
  ...
  async list(filter: {
    ...
    if (filter.product_id) {
      return this.listProducts(filter.product_id)
    } else if (filter.category_id) {
      return this.listCategories(filter.category_id)
    } else if (filter.collection_id) {
      return this.listCollections(filter.collection_id)
    }

    return []
  }

  private async listProducts(productIds: string | string[]) {
        ...
        if (result.data && result.data.length > 0) {
          ...
          results.push({
            ...
            variants: (product.variants || []).map((variant: any) => ({
              ...variant,
              id: `${variant.id}`,
              option_values: (variant.option_values || []).map((option_value: any) => ({
                ...option_value,
                id: `${option_value.id}`,
              })),
            })),
  ...
  private async listCategories(categoryIds: string | string[]) {
    const ids = Array.isArray(categoryIds) ? categoryIds : [categoryIds]
    const results: any[] = []

    for (const categoryId of ids) {
      try {
        const result = await this.client_.collection("product-categories").find({
          filters: {
            medusaId: {
              $eq: categoryId,
            },
        ...
        this.logger_.warn(this.formatStrapiError(error, `Failed to fetch category ${categoryId} from Strapi`))
      }
    }
  ...
  private async listCollections(collectionIds: string | string[]) {
    const ids = Array.isArray(collectionIds) ? collectionIds : [collectionIds]
    const results: any[] = []

    for (const collectionId of ids) {
      try {
        const result = await this.client_.collection("product-collections").find({
          filters: {
            medusaId: {
              $eq: collectionId,
            },
        ...
        this.logger_.warn(this.formatStrapiError(error, `Failed to fetch collection ${collectionId} from Strapi`))
      }
    }
