...
A [module](https://docs.medusajs.com/learn/fundamentals/modules) is a reusable package that provides functionalities related to a single feature or domain. Medusa integrates the module into your application without implications or side effects on your setup.
...
A [loader](https://docs.medusajs.com/learn/fundamentals/modules/loaders) is an asynchronous function that runs when the Medusa server starts. Loaders are useful for setting up connections to third-party services and reusing those connections throughout your module.
...
1. `container`: The [module container](https://docs.medusajs.com/learn/fundamentals/modules/container) that allows you to resolve and register module and Framework resources.
...
You resolve the [Logger](https://docs.medusajs.com/learn/debugging-and-testing/logging) and the Strapi client that you registered in the loader. You also store the module options for later use.
...
1. [Workflows](https://docs.medusajs.com/learn/fundamentals/workflows) that implement the logic to create product data in Strapi.
2. A [subscriber](https://docs.medusajs.com/learn/fundamentals/events-and-subscribers) that listens to the product creation event in Medusa and triggers the workflow.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve product option data.
...
1. Retrieve the product options in Medusa using the `useQueryGraphStep`.
   - This step uses [Query](https://docs.medusajs.com/learn/fundamentals/module-links/query) to retrieve data in Medusa across modules.
2. Prepare the option data to create using [transform](https://docs.medusajs.com/learn/fundamentals/workflows/variable-manipulation).
...
In a workflow, you can't manipulate data because Medusa stores an internal representation of the workflow on application startup. Learn more in the [Data Manipulation](https://docs.medusajs.com/learn/fundamentals/workflows/variable-manipulation) documentation.
...
- [acquireLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/acquireLockStep): Acquire a lock to prevent concurrent creation
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve product variant data.
- [releaseLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/releaseLockStep): Release the acquired lock
...
1. Acquire a [lock](https://docs.medusajs.com/infrastructure-modules/locking) to prevent concurrent creation of variants for the same product. This is necessary to handle both the product and variant creation events without duplications.
2. Retrieve the product variants in Medusa using the `useQueryGraphStep`.
3. Check if the product has been created in Strapi using [when](https://docs.medusajs.com/learn/fundamentals/workflows/conditions). If so, you:
...
In a workflow, you can't perform steps based on conditions because Medusa stores an internal representation of the workflow on application startup. Learn more in the [Conditions in Workflows](https://docs.medusajs.com/learn/fundamentals/workflows/conditions) documentation.
...
- [acquireLockStep](https://docs.medusajs.com/references/medusa-workflows/steps/acquireLockStep): Acquire a lock to prevent concurrent creation of product variants
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve product data.
...
- [updateProductsWorkflow](https://docs.medusajs.com/references/medusa-workflows/updateProductsWorkflow): Update the product's \`metadata\` with the Strapi ID.
...
1. Acquire a [lock](https://docs.medusajs.com/infrastructure-modules/locking) to prevent concurrent creation of variants for the same product. This is necessary to handle both the product and variant creation events. Otherwise, variants might be created multiple times.
...
Next, open the Medusa Admin dashboard and [create a new product](https://docs.medusajs.com/user-guide/products/create). Once you create the product, you'll see the following in the Medusa server logs:
...
The `clearProductCacheStep` clears the product cache in Medusa to ensure that updated data is served to clients. This is necessary as you'll enable [caching](https://docs.medusajs.com/infrastructure-modules/caching) later, which may cause stale data to be served to the storefront.
...
An [API route](https://docs.medusajs.com/learn/fundamentals/api-routes) is an endpoint that exposes business logic and commerce features to clients.
...
To ensure that webhook requests are coming from your Strapi application, you'll add a [middleware](https://docs.medusajs.com/learn/fundamentals/api-routes/middlewares) that validates the webhook requests.
...
1. Resolves the [API Key Module](https://docs.medusajs.com/commerce-modules/api-key)'s service.
...
The [Caching Module](https://docs.medusajs.com/infrastructure-modules/caching) is currently guarded by a feature flag. To enable it, add the feature flag and module in your `medusa-config.ts` file:
...
```bash
http:    POST /webhooks/strapi â† - (200) - 153.264 ms
```
